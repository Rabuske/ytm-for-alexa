...
import { parseSongArtists, parseSongArtistsRuns, parseSongRuns, parseSongRunsReturn } from './songs';
...
export type parseHomeReturn = {
    title: string;
    contents: (parseSongReturn | parseAlbumReturn | parseRelatedArtistReturn | parsePlaylistReturn | ({
        title: string;
        videoId: string;
        thumbnails: bT.thumbnails;
    } & Except<parseSongRunsReturn, 'album'> & {
        album?: parseSongRunsReturn['album'] | {
            title: string;
            browseId: string;
        };
    }))[];
